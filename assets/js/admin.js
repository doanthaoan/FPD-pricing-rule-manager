jQuery(document).ready(function($) {
    ...
    $('#group-form').submit(function(e) {
        ...
        const newGroupHtml = createGroupHtml(updatedGroup, groupIndex, true);
                ...
                <div class="image-item">
                    <label>
                        <input type="checkbox" class="image-checkbox" value="${image.ID}" 
                            data-title="${image.title}" ${window.selectedImages?.includes(image.ID) ? 'checked' : ''}>
    ...
    $('#save-category-groups').click(function() {
        ...
        if (!window.selectedAllImages && (!window.selectedImagesData || window.selectedImagesData.length === 0)) {
            hideLoading();
        ...
        $.ajax({
            ...
            data: {
                ...
                selected_images: window.selectedAllImages ? null : JSON.stringify(window.selectedImagesData),
    ...
    function createGroupHtml(group, index, isTemp) {
        const tempClass = isTemp ? 'temporary-item' : '';
        const tempBadge = isTemp ? '<span class="temp-badge">(Pending Save)</span>' : '';
            ...
            <div class="pricing-group ${tempClass}" data-index="${index}" data-category="${group.category || 'Uncategorized'}">
                <div class="group-header">
                    ...
                    <div class="group-details">
                        <span class="group-element">Element:</> ${group.data.target.elements}
                        </span>
                    </div>
                    <div class="group-actions">
                        ${isTemp ? '' : '<button class="button duplicate-group">Duplicate</button>'}
    ...
    function isTabOrFilterLink(element) {
        return $(element).hasClass('nav-tab') || 
